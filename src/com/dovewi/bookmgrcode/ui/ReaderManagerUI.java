...
class DeleteReaderPane extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		try{
			...
			JOptionPane.showConfirmDialog(null, "删除失败，请检查是否存在此用户","提示",JOptionPane.OK_OPTION);
		}finally{
			if(flag==true){
	...
	public void actionPerformed(ActionEvent arg0){
		if (arg0.getSource() == this.bt_get) {
			...
			try{
				...
				JOptionPane.showConfirmDialog(null, "获取信息出错","提示",JOptionPane.ERROR_MESSAGE);
			}
		} else if (arg0.getSource() == this.bt_commit) {
			...
			try{
				...
				JOptionPane.showConfirmDialog(null, "修改出错","提示",JOptionPane.CLOSED_OPTION);
			}finally{
